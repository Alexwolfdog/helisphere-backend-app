...
const getTeamList = async (req, res, next) => {
    try {
        if (!req.query.name && !req.query.owner) {
            const teams = await Team.find().populate('owner').populate('members').skip((req.query.page-1)*config.page.team.limit).limit(10);
            return res.status(200).json({
                success: true,
                statusCode: 200,
                data: teams,
        ...
        const teams = await Team.find(query).populate('owner').populate('members').skip((req.query.page-1)*config.page.team.limit);
        return res.status(200).json({
            success: true,
            statusCode: 200,
            data: teams,
